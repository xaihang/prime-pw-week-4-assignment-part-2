...
Requirements:
- Place the existing `<h1>` and `<p>` inside of a `header` element and add a background-color to that header. *DONE*
- Create a stylesheet.css file and import it into the index.html file. *DONE*
- Add a new div with a class name of "info". Inside that div add an `<h2> My name is YOUR NAME </h2>` Add a background-color and a text color of your choice. *DONE* and *DONE*
